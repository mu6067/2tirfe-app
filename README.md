...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', sans-serif; }
        ...
        .status-badge.status-active { background-color: rgba(46, 196, 182, 0.2) !important; color: var(--success) !important; }
        .status-badge.status-blocked { background-color: rgba(230, 57, 70, 0.2) !important; color: var(--danger) !important; }
    </style>
</head>
<body>
...
<script>
    ...
    function loadTenantApp() {
        ...
        document.getElementById('dispCapital').innerText = Number(d.capital).toFixed(2);
        document.getElementById('dispDailyProfit').innerText = Number(d.dailyProfit).toFixed(2);
        document.getElementById('dispMonthlyProfit').innerText = Number(d.monthlyProfit).toFixed(2);
        document.getElementById('dispDrawer').innerText = Number(d.drawerAmt).toFixed(2);
    ...
    function renderDebtTable() {
        ...
        debts.forEach(d => { tbody.innerHTML += `<tr><td>${d.name}</td><td>${d.detail}</td><td>${d.amt}</td></tr>`; });
    }
    ...
    function renderDrawerTable() {
        ...
        logs.forEach(l => { tbody.innerHTML += `<tr><td>${l.reason}</td><td>${l.amt}</td><td><span class="status-badge status-blocked">${l.status}</span></td></tr>`; });
    }
